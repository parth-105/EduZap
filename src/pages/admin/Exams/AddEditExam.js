...
function AddEditExam() {
  ...
  const [uploadedQuestions, setUploadedQuestions] = React.useState([]);
  ...
  const onFinish = async (values) => {
    try {
      ...
      let response;
      // If creating a new exam and questions are uploaded, include them
      if (!params.id && uploadedQuestions.length > 0) {
        response = await addExam({ ...values, questions: uploadedQuestions });
      } else if (params.id) {
          ...
          <Tabs defaultActiveKey="1">
            <TabPane tab="Exam Details" key="1">
              <Row gutter={[10, 10]}>
                ...
                {/* File upload for questions (only when adding a new exam) */}
                {!params.id && (
                  <Col span={24}>
                    <Form.Item label="Upload Questions (JSON)">
                      <input
                        type="file"
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            ...
                            const valid = parsed.every(q => q.name && q.options && q.correctOption);
                            if (!valid) {
                              setJsonError("Each question must have 'name', 'options', and 'correctOption'");
                              setUploadedQuestions([]);
                              return;
                            }
                      ...
                      {uploadedQuestions.length > 0 && (
                        <div style={{ marginTop: 10 }}>
                          <b>Preview ({uploadedQuestions.length} questions):</b>
                          <ul>
